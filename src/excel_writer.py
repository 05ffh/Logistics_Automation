...
每家发货公司独占一列：物流轨迹1 / 物流轨迹2 / 物流轨迹3 ...，
列号 N = 该公司单号在物流单号列首次出现的次序。缺列时紧跟最后一个物流轨迹列后插入。
只写查询到的公司(云驼/宁致/小满)对应列，不触碰其他公司(华洋/华运昌等)手填的列。

迁移清理：存量数据常把多家公司挤在物流轨迹1。写入查询公司到其正确列后，
