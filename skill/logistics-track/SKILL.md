...
**重要**: 查询前确保 Chrome 已通过 `.bat` 启动、对应标签页已打开（云驼=17track、宁致=nzhexp 且已登录、小满=xmsdwl）。查多个云驼单号时**优先一次全查**，不要每个单号单独开一个 python3 进程——在同一进程里循环更快。
...
1. **启动前**: 确认 Chrome 已启动、nzhexp 已登录、xmsdwl 和 17track 标签页正常
...
`9` = 自然位第 9；`/` = 未找到；`/（广告1）` = 未找到自然位、广告在第 1 页；`9（广告12）` = 自然位第 9、广告在第 1 和第 2 页；`9（广告12）（白广告3）` = 自然位第 9、普通广告在第 1/2 页、白色变体广告在第 3 页（仅 DE 刮水器使用）。多页广告合并为一个括号，页号连写无分隔符。
