...
| 云驼 | 999 | 17track.net | DOM 逐单 + 单条回退选"愿景征途" |
...
当用户粘贴半结构化物流文本（业务人员从 IM 发来）时，用数据录入模块解析并插入 Excel：
...
**自动处理**：
- 第 1 行日期 → 按日期排序定位插入行，格式 `m"月"d"日"`
- 第 2 行 `XX-XX` 格式识别为发货店铺，否则为发货公司
- 后续行按关键字匹配到对应列（单号、箱规、重量、FBA、交货仓等）
- 首次运行时自动初始化列模板（新增 货物名称/箱规/重量 列，重命名 箱数量→箱内数量）
- 所有格式（字体、边框、对齐）与原文件保持一致
- 固定后缀：箱规 `cm`、重量 `KG`、价格 `/KG`、发货日期 `左右`
